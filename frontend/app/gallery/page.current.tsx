...
const CATEGORIES = [
  ...
  { id: 'painting', name: 'Paintings', searchTerms: ['painting'] },
  { id: 'sculpture', name: 'Sculpture', searchTerms: ['sculpture'] },
  { id: 'still life', name: 'Still Life', searchTerms: ['still life'] },
  { id: 'portrait', name: 'Portraits', searchTerms: ['portrait'] },
  { id: 'landscape', name: 'Landscapes', searchTerms: ['landscape'] },
  { id: 'abstract', name: 'Abstract', searchTerms: ['abstract', 'modern'] },
];
  ...
  const fetchArtworks = async () => {
    ...
    try {
      ...
      if (selectedCategory !== 'all') {
        const categoryFilter = CATEGORIES.find(cat => cat.id === selectedCategory);
        if (categoryFilter) {
          filteredArtworks = allArtworks.filter((artwork: any) => {
            // Filter by searchTerm, classification, or department
            const searchTerm = categoryFilter.searchTerms?.[0] || categoryFilter.id;
            return artwork.searchTerm?.includes(searchTerm) ||
                   artwork.classification?.includes(searchTerm) ||
                   artwork.department?.toLowerCase().includes(searchTerm.toLowerCase());
          });
        }
      }
      
      // Randomly select 20-30 artworks and convert to our format
      const shuffled = filteredArtworks.sort(() => 0.5 - Math.random());
      const selectedArtworks = shuffled.slice(0, 25);
      
      const validArtworks: Artwork[] = selectedArtworks
        .filter((artwork: any) => artwork.cloudinaryUrl && artwork.title && artwork.artist)
        .map((artwork: any) => ({
          id: artwork.objectID || artwork.id,
          title: artwork.title,
          artist: artwork.artist.split('\n')[0] || 'Unknown Artist', // Take first line if multi-line
          year: artwork.date || 'Unknown Date',
          imageUrl: artwork.cloudinaryUrl,
          museum: artwork.source || artwork.department || 'Museum Collection',
          medium: artwork.medium || 'Unknown Medium',
          department: artwork.department || 'Art',
          culture: artwork.culture,
          period: artwork.period,
          dimensions: artwork.dimensions,
          museumUrl: artwork.metUrl,
          isPublicDomain: true,
