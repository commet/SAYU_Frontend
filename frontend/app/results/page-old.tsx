...
import { Heart, Sparkles, Map, Share2, Palette, User, Zap, Target, Sprout } from 'lucide-react';
...
function ResultsContent() {
  ...
  // gamification removed - will use simple data
    ...
    <div className="min-h-screen bg-off-white" data-personality={results.personalityType}>
      ...
      <section className="max-w-4xl mx-auto px-lg mb-lg">
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.6, duration: 0.6 }}
          className="bg-gradient-to-br from-indigo-500/10 to-purple-500/10 rounded-xl p-lg border border-indigo-200 relative overflow-hidden"
        >
          <div className="absolute top-0 right-0 w-48 h-48 bg-gradient-to-br from-indigo-400/20 to-purple-400/20 rounded-full blur-3xl -mr-24 -mt-24"></div>
          <div className="relative">
              ...
              <div className="flex items-center gap-sm">
                ...
                <div>
                  <h3 className="font-display text-lg font-semibold text-black leading-snug">
                    {language === 'ko' ? '16가지 예술 감상 유형' : '16 Art Appreciation Types'}
                  </h3>
                  <p className="text-sm text-dark-gray mt-1 leading-relaxed">
                    {language === 'ko' ? '다른 유형들도 궁금하신가요?' : 'Curious about other types?'}
                  </p>
                </div>
              </div>
            </div>
            ...
            <p className="text-sm text-dark-gray mb-md leading-relaxed">
              {language === 'ko' 
                ? '당신의 유형 외에도 15가지의 독특한 예술 감상 스타일이 있습니다. 각 유형의 특징과 동물 캐릭터를 만나보세요.'
                : 'Beyond your type, there are 15 other unique art appreciation styles. Discover their characteristics and animal characters.'}
            </p>
            
            <button
              onClick={() => router.push('/personality-overview')}
              className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-lg py-sm rounded-md hover:from-indigo-700 hover:to-purple-700 transition-all duration-base font-medium text-sm flex items-center gap-xs"
            >
              <Palette size={16} />
              {language === 'ko' ? '모든 유형 둘러보기' : 'Explore All Types'}
            </button>
          </div>
        </motion.div>
      </section>
